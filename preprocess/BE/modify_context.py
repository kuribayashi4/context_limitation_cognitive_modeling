...
def attach_context(lines, modify_context, ngram=1000):
    ...
    for sent_id, sent in groupby(lines, key=lambda x: x["sentN"]):
        sent = list(sent)
...
def main(args):
    ...
    with open("data/BE/fpt-log.csv.annotation.filtered.csv") as f:
        reader = csv.DictReader(f, delimiter="\t", quotechar='"')
        for article_id, lines in groupby(reader, lambda x: x["article"]):
            article_id = "_".join(article_id.split("_")[2:])
    ...
    json.dump(
        article2pieces_context,
        open(f"data/BE/{filename}.json", "w"),
        ensure_ascii=False,
    )
